                        ...
                        <tr
                            ...
                            <td
                                ...
                                <a
                                    ...
                                    <b-icon
                                        :style="{'margin-left': row.level * 20 + 'px'}"
                                        :icon="detailIcon"
                                        :pack="iconPack"
                                        both
...
export default {
    ...
    computed: {
        ...
        */
        visibleData() {
            ...
            const rootNodes = this.newData.filter(obj => obj[parentField] == null)
            let visibleData = this.newData.filter(obj => obj.visible == true)
            if (!this.paginated) return visibleData
            const currentPage = this.newCurrentPage
            const perPage = this.perPage
            if (rootNodes.length <= perPage) {
                return visibleData
            } else {
                const start = (currentPage - 1) * perPage
                const end = parseInt(start, 10) + parseInt(perPage, 10)
                const filteredRootNodes = rootNodes.slice(start, end)
                let data = []
                filteredRootNodes.forEach(obj => {
                  data = data.concat(this.getDescendants(obj))
                })
                return data.filter(obj => obj.visible == true)
            }
        },
        visibleColumns() {
        ...
        */
        showDetailRowIcon() {
            return (this.detailed && this.showDetailIcon || this.hierarchical)
        },
        /**
    ...
    watch: {
        ...
        *   1. Update internal value, add properties for collapsing if hierarchical.
        ...
        */
        data(value) {
            if(this.hierarchical)
                this.newData = this.enrichTreeData(value)
            else
                this.newData = value
            ...
            if (!this.backendPagination) {
                const parentField = this.parentField
                this.newDataTotal = this.newData.filter(obj => obj[parentField] == null).length
            }
        },
        /**
    ...
    methods: {
        ...
        */
        ...
        getDescendants(obj){
          const parentField = this.parentField
          const children = []
          for(const i in this.newData){
            const element = this.newData[i]
            if(element[parentField] == obj.id){
              children.push(element)
              this.getDescendants(element).forEach(o => {
                children.push(o)
              })
            }
          }
            ...
            for(const i in this.newData){
                ...
                if(element[parentField] == obj.id){
                    if(forced == undefined){
                        ...
                        element.visible = forced
                    }
                    if(element.visible == false){
                        this.closeDetailRow(element)
                    }else{
                        this.openDetailRow(element)
                    }
                    this.toggleNode(element, false)
                }
            }
        },
        toggleDetails(obj) {
